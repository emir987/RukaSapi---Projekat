...
$isAll = 0;
if (!is_null($category[0]) && !is_null($category[1]) && !is_null($category[2])) {
    $query = "SELECT * FROM pet WHERE status like 'aktivan'";
    $isAll = 1;
} elseif (!is_null($category[0]) && !is_null($category[1])) {
    $query = "SELECT * FROM pet where (category like '$category[0]' or category like '$category[1]') AND status like 'aktivan'";
    $isAll = 0;
} elseif (!is_null($category[0]) && !is_null($category[2])) {
    $query = "SELECT * FROM pet where (category like '$category[0]' or category like '$category[2]') AND status like 'aktivan'";
    $isAll = 0;
} elseif (!is_null($category[1]) && !is_null($category[2])) {
    $query = "SELECT * FROM pet where (category like '$category[1]' or category like '$category[2]') AND status like 'aktivan'";
    $isAll = 0;
} elseif (is_null($category[1]) && is_null($category[2])) {
    $query = "SELECT * FROM pet where category like '$category[0]' AND status like 'aktivan'";
    $isAll = 0;
} elseif (is_null($category[0]) && is_null($category[2])) {
    $query = "SELECT * FROM pet where category like '$category[1]' AND status like 'aktivan'";
    $isAll = 0;

} elseif (is_null($category[0]) && is_null($category[1])) {
    $query = "SELECT * FROM pet where category like '$category[2]' AND status like 'aktivan'";
    $isAll = 0;
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $ownerQuery = "SELECT * from users WHERE id = '$ownerID'";    
        $ownerResult = $connect->query($ownerQuery)->fetch_assoc();
        
        $pet["address"] = $ownerResult["address"];
        $pet["ownerName"] = $ownerResult["name"] . " " . $ownerResult["surname"]; 
    ...
    while ($row = $resultNewPets->fetch_assoc()) {
        ...
        $newPet["image"] = $imagesResult["image"];
