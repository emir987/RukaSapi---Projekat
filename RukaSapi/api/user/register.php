...
$photo = htmlspecialchars(basename($_FILES["slika"]["name"]));
$target_dir = 'C:\/xampp\/htdocs\/RukaSapi\/slike_profil\/';
...
if (!$error) {
    ...
    $upit = $connect->prepare("INSERT INTO users (name, surname, email, password, phone, address,zip, photo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $upit->bind_param("ssssssss", $name, $surname, $email, $password, $phone, $address, $zip, $photo);
    $result = $upit->execute();
    $upit->close();
